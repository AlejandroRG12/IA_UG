...
int main(){ 
    ...
    printf("\nSe creo el ENJAMBRE");
    InicializarEnjambre(Enj,0.06,2.0,2.0,NumeroMaximoDeIteraciones,LimiteInferior,LimiteSuperior);
...
void ActualizarVelocidad(ENJAMBRE *Enj){
    ...
    { 
        ...
        Enj->Part[i].Vi[j] += (Enj->w*Enj->Part[i].Vi[j]) + (Enj->C1*Y1*(Enj->Part[i].Pi[j]-Enj->Part[i].Xi[j])) + (Enj->C2*Y2*(Enj->Part[Enj->IdPg].Pi[j]-Enj->Part[i].Xi[j]));
