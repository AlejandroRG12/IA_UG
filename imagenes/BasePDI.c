...
{ //Declarar un Puntero a imagen
  ...
  /* //recorer los datos de la imagen completo
  for(int i=0; i<100; i++){
    printf("\n");
    for(int j=0; j<100; j++){
      printf(" %g", Img1->imx[i+Img1->ancho+j]);
    }
  } */
  ...
  for(int i=0; i<Img1->alto; i++){
    for(int j=0; j<Img1->ancho; j++){
      Img1->imx[i*Img1->ancho+j]= 255- Img1->imx[i*Img1->ancho+j];
    }
  } */

  /* //binarisado de la imagen (umbralizado)
  for(int i=0; i<Img1->alto; i++){
    for(int j=0; j<Img1->ancho; j++){
      if(Img1->imx[i*Img1->ancho+j] <128){
  ...
  gcPutImgBmp("Resultado.bmp",Img1);
